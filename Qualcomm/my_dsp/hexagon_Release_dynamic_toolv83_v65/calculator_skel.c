...
#else
const Type types[1] = {{0x4,{{(const uintptr_t)0,(const uintptr_t)1}}, 2,0x4}};
#endif
...
extern "C" {
#endif
static __inline int _skel_method(int (*_pfn)(char*, uint32_t, uint64_t*), uint32_t _sc, remote_arg* _pra) {
   remote_arg* _praEnd;
   char* _in0[1];
   uint32_t _in0Len[1];
   uint64_t _rout1[1];
...
__QAIC_SKEL_EXPORT int __QAIC_SKEL(calculator_skel_invoke)(uint32_t _sc, remote_arg* _pra) __QAIC_SKEL_ATTRIBUTE {
